...
function imgDelay() {
    for (var i = 0; i < imgList.length; i++) {
        ...
        curImg.onload = function () {
            ...
            if (this.i === 0) {
                //因为图片是放在div中，所以我们要改变的是父级元素div的层级关系和透明度
...
function move() {
    ...
    //循环所有的图片，判断图片的索引是否等于step，等于step的那张图片，先把他的层级关系设为1，然后在让他的透明度增加到1，
    setImg();
}
function setImg() {
    //图片的切换
    for (var i = 0; i < imgList.length; i++) {
        if (i === step) {
            ...
            animation(imgList[i].parentNode, {opacity: 1}, 1000, function () {
                //获取当前元素的所有兄弟,得到一个数组，循环数组，让所有兄弟的透明度都变成0
